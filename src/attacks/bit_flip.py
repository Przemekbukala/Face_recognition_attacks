...
from face_recognition_model.people_comparer import dget_embedding, compare_embeddings
...
def bit_flip_attack(photo1: np.ndarray, photo2: np.ndarray) -> np.ndarray:
    """Perform a bit flip attack on the input images. It should try to change some bits, so photos wont be recognized as the same person. Genetic algorithm will be used to do it."""
    return _train_attack(photo1, photo2)
...
def bit_flip_based_on_genome(photo: np.ndarray, genome: np.array) -> np.ndarray:
    ...
    for i in range(0, len(genome), 2):
        x = genome[i]
        y = genome[i + 1]
        attacked_photo[x, y] = 255 - attacked_photo[x, y]  # Flip the pixel value
...
def _choose_population_to_futher_training(population : np.array, photo1: np.ndarray, photo2_emb: np.ndarray) -> np.array:
    """Choose a subset of the population for further training."""
    
    results_of_similarity = np.array([_evaluate_similarity(bit_flip_based_on_genome(photo1, genome), photo2_emb) for genome in population])
...
def _choose_photo_embedding_from_many(photos: list) -> tuple[int, np.ndarray]:
    """Choose the photo from the set of photos that is most similar to the original one."""
    index = 0
    cache = []
    while True:
        if index >= len(cache):
            cache.append(dget_embedding(photos[index]))
        yield index, cache[index]
        index = (index + 1) % len(photos)

def _train_attack(photos : list) -> np.ndarray:
    """Train the bit flip attack using a genetic algorithm."""
    photo1 = photos[0]
    photo_generator = _choose_photo_embedding_from_many(photos)
    ...
    for i in range(get_attack_params("bitflip").get("epochs")):
        photo2_index, photo2_emb = next(photo_generator)
        population = _choose_population_to_futher_training(population, photo1, photo2_emb)
        new_population = []
        ...
        best_fitted_genome = max(population[0], best_fitted_genome, key=lambda genome: _evaluate_similarity(bit_flip_based_on_genome(photo1, genome), photo2_emb))
        best_similarity = _evaluate_similarity(bit_flip_based_on_genome(photo1, best_fitted_genome), photo2_emb)
        training_log.append((i, photo2_index, float(best_similarity)))
        logger.debug(f"Best similarity: {best_similarity} in epoch {i}")
        population = np.array(new_population)
    results_dir = _ensure_results_dir()
    training_log_path = results_dir / "training_log.csv"
    with training_log_path.open("w", newline="") as handle:
        writer = csv.writer(handle)
        writer.writerow(["epoch", "photo_index", "best_similarity"])
...
if __name__ == "__main__":
    ...
    ref_emb = dget_embedding(photos[0])
    before_results = []
    for photo in photos[1:]:
        before_results.append(compare_embeddings(ref_emb, dget_embedding(photo)))
    ...
    attacked_emb = dget_embedding(attacked)
    after_results = []
    for photo in photos[1:]:
        after_results.append(compare_embeddings(attacked_emb, dget_embedding(photo)))
